...
class Echo_dynamic_video(Dataset):
    """EchoNet-Dynamic Dataset.
    Args:
    ...
    def __getitem__(self, index):
        ...
        video, fps = loadvideo(video, self.grey).astype(np.float32)
...
class Echo_dynamic_seg(Dataset):
    """EchoNet-Dynamic Dataset.
    Args:
        root (string): Root directory of dataset (defaults to `echonet.config.DATA_DIR`)
        split (string): One of {``train'', ``val'', ``test'', ``all'', or ``external_test''}
    ...
    def __getitem__(self, index):
        ...
        if self.clips == "all" and self.validation is False:
            # Take all possible clips of desired length
            start = np.arange(f - (length - 1) * self.period)
        elif self.validation is False:
        ...
        return video.squeeze(1), np.expand_dims(target, axis=0), fps
...
if __name__ == '__main__':
    ...
    print(data_loader)
    for targets, targets_gt, fps in train_loader:
        pass
