...
class Echo_dynamic(Dataset):
    """EchoNet-Dynamic Dataset.
    Args:
        root (string): Root directory of dataset (defaults to `echonet.config.DATA_DIR`)
        split (string): One of {``train'', ``val'', ``test'', ``all'', or ``external_test''}
        sampled_fps (int, optional): Sampling fps for taking a clip from the video (i.e. for each second, sampled_fps slides are taken)
            Defaults to 10.
    ...
    def __init__(self, root,
                sampled_fps=10, period=1, phase='ode',
    ...
    def __getitem__(self, index):
        ...
        c, f, h, w = video.shape
        self.period = fps // self.sampled_fps
        ...
        if self.phase == 'ode':
            ...
            if self.validation is False:
                ...
                if self.target_type[0] == 'LargeTrace':
                    start = np.int64(self.frames[self.fnames[index]][-1])
                elif self.target_type[0] == 'SmallTrace':
                    start = np.int64(self.frames[self.fnames[index]][0])
            elif self.validation:
                if self.target_type[0] == 'LargeTrace':
                    start = np.int64(self.frames[self.fnames[index]][-1])
                elif self.target_type[0] == 'SmallTrace':
                    start = np.int64(self.frames[self.fnames[index]][0])
        indices = start + self.period * np.arange(length)
        video = video[:, indices, :, :]
        ...
        return video, np.expand_dims(target, axis=0), fps, timestamps
...
if __name__ == '__main__':
    data_loader = Echo_dynamic(
            root='/data/liujie/data/echocardiogram/EchoNet-Dynamic', split='ALL', target_type="SmallTrace"
        )


    train_loader = DataLoader(data_loader, batch_size=1, shuffle=False, num_workers=1)
    # print(data_loader)
    for targets, targets_gt, fps, timestamps in train_loader:
        print(timestamps)
        print(targets.shape, targets_gt.shape, timestamps.shape, fps)
        # print(targets.shape, targets_gt.shape, fps)
        # import numpy as np
